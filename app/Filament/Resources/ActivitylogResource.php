...
class ActivitylogResource extends Resource
{
    ...
    {
        return [
            'logactivity.index' => 'Lihat Log Aktivitas',
        ];
    }

    public static function canAccess(): bool
    {
        // Combine HexaLite permission with plugin/policy checks
        $hexaAllowed = true;
        try {
            if (function_exists('hexa')) {
                $hexaAllowed = hexa()->can('logactivity.index');
            }
        } catch (\Throwable $e) {
            $hexaAllowed = true; // if HexaLite not available, don't block access
        }

        $policy = Gate::getPolicyFor(static::getModel());
        $baseAllowed = ($policy && method_exists($policy, 'viewAny'))
            ? static::canViewAny()
